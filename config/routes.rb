...
Kassi::Application.routes.draw do
  ...
  #static page callbacks
  get '/ambassador/' => 'become_ambassador#ambassador', as: 'become_ambassador'
  get '/promote_your_business/' => 'promote_business#business', as: 'promote_business'
  get '/promote_your_destination/' => 'promote_destination#destination', as: 'promote_destination'

  #destination redirects
  get '/canberra', to: redirect('/en/destination/canberra', status: 302)
  #destination callbacks
  get '(/:locale)/destination/list' => 'destinations_list#destinations', as: 'destinations_list'
  get '(/:locale)/destination/canberra' => 'destination#destination', as: 'destination'
  ...
  get "(/:locale)/listings/related_form_content/:id" => "listings#related_content", :as => :related_form_content
  get "(/:locale)/listings/:id/related/" => "listings#related_listing", :as => :related_listing
  get "(/:locale)/listings/:id/related_form/" => "listings#related_listing_form", :as => :related_form
  get "/related_confirm" => "related_listings#related_content_save", :as => :related_confirm
