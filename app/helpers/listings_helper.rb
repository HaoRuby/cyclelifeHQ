module ListingsHelper
  ...
  def listing_form_menu_titles_related()
    titles = {
      "relcategory" => t("listings.form.related.select_category"),
      "relsubcategory" => t("listings.form.related.select_subcategory"),
      "rellisting_shape" => t("listings.form.related.select_transaction_type")
    }
  end
...
#CLHQ
  ...
  def first_listing_image(listing, &block)
    Maybe(listing)
      .listing_images.first
      .map { |images| image}[:small_3x2].each { |url|
      block.call(url)
    }
