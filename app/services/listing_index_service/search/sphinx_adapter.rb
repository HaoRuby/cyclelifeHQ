module ListingIndexService::Search
  ...
  class SphinxAdapter < SearchEngineAdapter
    ...
    def search(community_id:, search:, includes:, geo:)
    ...
    def search_with_sphinx(community_id:, search:, included_models:, includes:, geo:)
      ...
      if perform_numeric_search && numeric_search_match_listing_ids.empty?
        ...
        if (includes.include?(:location) && !geo.blank?) 
          geo = geo.split(',')
          geo.map! {|n| (n.to_f*3.141592654)/180.0 }
          geodist_hash = {:geodist => 0..100000}
          with = with.merge(geodist_hash)
