...
window.ST.initializeListingShapeForm = function(formId) {
  ...
  var deeplinkChanged = function(currentEl) {
    var enabled = currentEl.is(':checked');
    if(enabled) {
      toggleOnlinePaymentEnabled(false);
      if (!$('.js-price-enabled').is(':checked')) {
  ...
  var priceChanged = function(currentEl) {
    var enabled = currentEl.is(':checked');
    if(enabled) {
      if (!$('.js-deeplink-enabled').is(':checked')) {
        toggleOnlinePaymentEnabled(true);
      }
      toggleUnitsEnabled(true);
    } else {
  ...
  var toggleDeeplinkEnabled = function(enabled) {
    toggleCheckboxEnabled($(".js-deeplink-enabled"), enabled);
  };

  var togglePriceEnabled = function(enabled) {
    toggleCheckboxEnabled($(".js-price-enabled"), enabled);
  };
  ...
  $('.js-deeplink-enabled').change(function() {
    deeplinkChanged($(this));
  });
  ...
  // Run once on init
  toggleUnitsEnabled(false);
  deeplinkChanged($('.js-deeplink-enabled'));
