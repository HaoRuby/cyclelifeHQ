...
class ListingsController < ApplicationController
  ...
  before_filter :only => [ :edit, :related, :edit_form_content, :update, :close, :follow, :unfollow ] do |controller|
    controller.ensure_logged_in t("layouts.notifications.you_must_log_in_to_view_this_content")
  end

  before_filter :only => [ :new, :related, :new_form_content, :create ] do |controller|
  ...
  def show
    ...
    @related_list = []
    @rel_image = []
    @rlisting_show = RelatedListing.where('listing_id' => @listing.id).order(:related_order)
    rlist_array = []
    related_image = []
    unless (@rlisting_show.nil?)
      @rlisting_show.each do |rlist|
        rlist_array = rlist.related_listing_ids.split

        rlist_array.each do |rlist|
          related_image.push(Listing.find(rlist))
        end
        @related_list.push(related_image)
        related_image = []
      end
    end
    ...
    view_locals = {
      ...
      related_listings_images: @related_listings_images
    }
  ...
  def related_listing
    ...
    if (!params[:commit].nil?)
      if (params[:commit] == "Return to listing")
        redirect_to listing_path(@listing.id) and return
      end

      unless (params[:commit] == "New related listing")
        add_new = 0

        paramval = params[:commit]
        move_id = paramval.key("Move listing up")
        if (move_id.nil?)
          move_id = paramval.key("Move listing down")
        end

        up_down = params[:commit][move_id]
        if (up_down == "Move listing up" || up_down == "Move listing down")
          if (up_down =="Move listing up")
            #move up
            if (!move_id.nil?)
              #get current order
              rcurrent = RelatedListing.find(move_id)
              rlist = rcurrent.related_order
              rorig = rlist
              if (rlist >= 2)
                rlist -= 1
                #get order of one above
                rswap = RelatedListing.where('listing_id' => @listing.id).find_by related_order: rlist
                #update current
                rcurrent.update_attribute(:related_order, rlist)
                #update one above
                rswap.update_attribute(:related_order, rorig)
              end
            end
          else
            #move down
          ...
          del_id = paramval.key("Delete this listing")
          del = params[:commit][del_id]
          if (del == "Delete this listing")
            if (!del_id.nil?)
              rlist = RelatedListing.find(del_id).destroy

            #add code to reorder numbers in case an internally ordered entry deleted
              redirect_to listing_path(@listing.id) and return
            end
          end
        end
      ...
      if (params[:commit] == "New related listing")
        #get highest order for related listings
        @rl_order = 0
        rel_ord = RelatedListing.where('listing_id' => @listing.id).maximum('related_order')
        unless (rel_ord.nil?)
          @rl_order = rel_ord + 1
        else
          @rl_order = 1
        ...
        add_new = 1
      elsif (params[:commit] == "Move listing up" || params[:commit] == "Move listing down")
        add_new = 0
      end
    end
    ...
    related_list = []
    @all_related_list = []
    already_related = RelatedListing.where('listing_id' => @listing.id).order(:related_order)
    already_related.each do |alrel|
      listids = alrel.related_listing_ids.split
      listids.each do |lid|
        related_list.push(Listing.find(lid))
      end
      @all_related_list.push(related_list)
      related_list = []
    end

    if (already_related.size == 0 || (add_new > 0 && add_new < 3))
      if (already_related.size == 0 || add_new == 1)
        render "listings/related_listing", locals:
             {
               category_tree: category_tree,
               categories: @current_community.top_level_categories,
               subcategories: @current_community.subcategories,
        ...
        render "listings/related_listing", locals:
             {
               ...
               subcategories: @current_community.subcategories,
               shapes: get_shapes,
               category_id: rlist.category,
               subcategory_id: rlist.subcategory,
               shape_id: rlist.shape,
               form_content: form_locals(shape)
             }
      end
    elsif (add_new == 3)
      ...
      render "listings/rellist", locals:
          {
            arelated: already_related
          }
    end
  end
  ...
  def related_content
    return redirect_to action: :related_listing unless request.xhr?
    ...
    @related_listing_shape = params['listing_shape']
    @related_subcategory = params['subcategory']
    @related_category = params['category']

    related_form_content
  ...
  def update
    if (!params[:rlist].blank?)
      rlist = Hash.new
      rlist[:category] = params[:listing][:category_id]
      rlist[:subcategory] = params[:listing][:subcategory_id]
      rlist[:shape] = params[:listing][:listing_shape_id]
      rlist[:heading] = params[:rlist]
      rlist[:listing_id] = @listing.id
      rlist[:related_listing_ids] = params[:listing][:related_listing_ids]
      rlist[:related_order] = params[:rorder]
      RelatedListing.create(rlist)

      @listing.related_listing = nil
      @listing.update_attribute(:related_listing, nil)

      flash[:notice] = "The related listing was created."
      redirect_to @listing, status: 303 and return
    end
  ...
  def related_form_content
    ...
    #creates a new related listing list
    @related_list = []

    latitude = Location.where('listing_id' => @listing.id).all[0].latitude
    longitude = Location.where('listing_id' => @listing.id).all[0].longitude
    #geocode
    distance=30 #in km
    close_listings = Location.get_coords(latitude,longitude,distance)#.limit(4)
    related_count = 0
    close_listings.each do | close |
      if (close.listing_id.blank? || close.listing_id == @listing.id)
        next
      end

      the_listing = Listing.find(close.listing_id)
      if (the_listing.category_id == @listing.category_id && the_listing.listing_shape_id == @listing.listing_shape_id)
        ...
        @related_list.push(the_listing)
      end
    end
    @id_related_list = @related_list.map{ |i| i.id }
    #get highest order for related listings
    rel_ord = RelatedListing.where('listing_id' => @listing.id).maximum('related_order')
    unless (rel_ord.nil?)
      @rl_order = rel_ord + 1
    ...
    if allow_posting
      render :partial => "listings/form/related_form_content", locals:
        form_locals(shape).merge(
               run_js_immediately: true,
               related_list: @related_list,
               id_related_list: @id_related_list,
               related_order: @rl_order
             )
    else
      render :partial => "listings/payout_registration_before_posting", locals: { error_msg: error_msg }
  ...
  def ensure_authorized_to_view
    ...
    unless @listing.visible_to?(@current_user, @current_community) || (@current_user && @current_user.has_admin_rights?)
      if @current_user
        ...
        response.headers["headers"] = '<link href="'+request.host_with_port+'/en/login" rel="canonical" />'
