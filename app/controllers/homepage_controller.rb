...
class HomepageController < ApplicationController
  ...
  def index
    ...
    if (main_search == :location || main_search == :keyword_and_location)
      if (params[:lc].blank? && !params[:lq].blank?) 
        coords = Geocoder.coordinates(params[:lq])
        lc = "#{coords[0]},#{coords[1]}"
        params[:lc] = lc
        Rails.logger.warn "LC: #{params[:lc]}"
      end
    end

    Rails.logger.warn "LC: #{params[:lc]} LQ: #{params[:lq]}"
  ...
  def search_coordinates(latlng)
    Rails.logger.warn "LATLNG #{latlng}"
    lat, lng = latlng.split(',')
    Rails.logger.warn "LAT: #{lat} LNG: #{lng}"
