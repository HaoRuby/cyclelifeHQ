class Admin::CustomFieldsController < Admin::AdminBaseController
  ...
  HozirontalFieldEntity = EntityUtils.define_builder(*HorizontalFieldSpec)
  ...
  def create
    ...
    Rails.logger.warn "CA: #{params[:custom_field][:category_attributes].inspect}"
    Rails.logger.warn "CSTF: #{@custom_field.inspect}"
    success =
      if valid_categories?(@current_community, params[:custom_field][:category_attributes])
        if @custom_field.type == "HorizontalField"
  ...
  def valid_categories?(community, category_attributes)
    return true
    Rails.logger.warn "CAT: #{category_attributes}"
